  ...
  ({ className, children, ...props }, ref) => {
    ...
    return (
      <button
        ...
        className={`flex h-10 w-full items-center justify-between rounded-md border border-gray-300 bg-white dark:bg-slate-800 px-3 py-2 text-sm ring-offset-white placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-[#018a83] focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 dark:border-slate-700 dark:text-slate-100 dark:focus:ring-offset-slate-900 text-gray-800 ${className}`}
        {...props}
      >
        {children}
...
const SelectContent = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, children, ...props }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className={`absolute z-50 mt-1 w-full rounded-md border border-gray-200 bg-white shadow-lg dark:bg-slate-800 dark:border-slate-700 ${className}`}
        {...props}
      >
        {children}
      </div>
  ...
  ({ className, children, value, ...props }, ref) => {
    ...
    return (
      <div
        ...
        className={`relative flex w-full cursor-pointer select-none items-center rounded-sm py-1.5 px-3 text-sm outline-none hover:bg-gray-100 dark:hover:bg-slate-700 dark:text-slate-100 text-gray-800 ${className}`}
        {...props}
      >
        {children}
      </div>
