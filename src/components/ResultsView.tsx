...
const ResultsView: React.FC<ResultsViewProps> = ({ quizData, onStartQuiz, onReset }) => {
  ...
  const StatCard: React.FC<{ icon: React.ReactNode; label: string; value: string | number }> = ({ icon, label, value }) => (
    <div className="bg-gray-100 dark:bg-slate-700 p-4 rounded-lg flex items-center space-x-3 transition-colors">
    ...
    <Card className="w-full max-w-3xl mx-auto shadow-lg">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="text-center pt-4">
            <h3 className="font-semibold text-xl dark:text-white">Ready to test your knowledge?</h3>
        </div>
      </CardContent>
      <CardFooter className="flex flex-col sm:flex-row gap-4">
        ...
        <Button onClick={onReset} variant="outline" className="w-full sm:w-auto dark:border-slate-600 dark:text-gray-300 dark:hover:bg-slate-700">
          Create New Quiz
        </Button>
      </CardFooter>
    </Card>
